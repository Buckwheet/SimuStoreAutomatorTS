...
export async function launchBrowser(): Promise<void> {
    ...
    try {
        ...
        try {
            await page.goto('https://store.play.net/store/purchase/gs', { waitUntil: 'networkidle2' });
        } catch (e) {
...
export async function purchaseItem(itemId: string, cost: number, quantity: number, onProgress?: (current: number, total: number) => void): Promise<{ status: string }> {
    if (!page) throw new Error("Browser not launched");

    console.log(`Starting purchase loop for Item ${itemId}, Cost ${cost}, Quantity ${quantity}`);

    for (let i = 0; i < quantity; i++) {
        console.log(`Purchase ${i + 1}/${quantity}...`);

        // Execute the fetch in the browser context to use existing cookies/session
        const success = await page.evaluate(async (id: string, costVal: number) => {
            try {
                const response = await fetch(`https://store.play.net/store/PurchaseItemConfirmed?id=${id}&qty=1&confirmation_item=yes&cost=${costVal}`, {
                    "headers": {
                        "accept": "*/*",
...
export async function purchaseCart(cartItems: CartItem[], onProgress?: (current: number, total: number, msg: string) => void): Promise<{ status: string }> {
    if (!page) throw new Error("Browser not launched");

    console.log(`Starting cart purchase for ${cartItems.length} unique items.`);

    let totalItems = 0;
    cartItems.forEach(i => totalItems += i.quantity);
    let currentItem = 0;
    ...
    for (const item of cartItems) {
        ...
        for (let i = 0; i < item.quantity; i++) {
            currentItem++;
            if (onProgress) onProgress(currentItem, totalItems, `Buying ${item.name || item.id} (${i + 1}/${item.quantity})`);
            ...
            if (currentItem < totalItems) await new Promise(r => setTimeout(r, 2000));
        }
    }
